...
VISPSubscriber::VISPSubscriber(Controller & ctl,
  ...
  ctl_(ctl)
{
  ...
  for(auto k : objects.keys())
  {
    ...
    if(objects(k).has("load") && simulation)
    {
      ...
      auto object = mc_rbdyn::RobotLoader::get_robot_module(
          std::string("env"), std::string(mc_rtc::MC_ENV_DESCRIPTION_PATH), k);
    ...
    updateThread_ = std::thread([this]()
      {
        ...
        while(ros::ok() && running_)
        {
          ...
          for(auto & o : objects_)
          {
            ...
            {
              X_0_object = ctl_.robots().robot(o.second.robot).surface(o.second.tf).X_0_s(ctl_.robots().robot(o.second.robot));
            }
            else
            {
  ...
  else
  {
    for(auto & o : objects_)
    {
      ...
      boost::function<void(const msg_t&)> callback_ = [this,&o](const msg_t & msg)
      {
        ...
        X_camera_object.rotation() = Eigen::Matrix3d(Eigen::Quaterniond(rot.w, rot.x, rot.y, rot.z).inverse().normalized());
...
void VISPSubscriber::newObject(const std::string & name)
{
  ...
  ctl_.logger().addLogEntry("VISPObjects_" + name, [this, name]() -> const sva::PTransformd & { return objects_.at(name).pos; });
  ctl_.logger().addLogEntry("VISPObjects_" + name + "_World", [this, name]() -> const sva::PTransformd & { return objects_.at(name).posW; });
  auto gui = ctl_.gui();
  if(!gui) { return; }
  gui->addElement({"VISP", "Objects"},
    mc_rtc::gui::Transform(name,  [this, name]() { return objects_.at(name).posW; },
